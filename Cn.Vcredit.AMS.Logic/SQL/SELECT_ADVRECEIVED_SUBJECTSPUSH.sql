...
CONVERT(NVARCHAR(7), DATEADD(MONTH, 1, t.BillMonth + '/01'), 111) AS BillMonth, --取最后一期非提前清贷的后一个月
CASE Subject 
	WHEN 25 THEN '担保违约金'
	WHEN 28 THEN '加收利息'
	WHEN 29 THEN '加收服务费'
	...
	WHEN 31 THEN '剩余本金' 
END AS Item,
'清贷' AS [Type], CONVERT(DATE, ClearLoanTime) AS DeductionDate, '' Explain, CONVERT(INT, MIN(BillItemID)) AS [Identity] --因为没有CONCAT_GROUP，所以Explain只能放弃
FROM ViewReceived r
LEFT JOIN (
...
AND EXISTS (
	SELECT * FROM Received WHERE BillItemID = r.BillItemID AND ReceivedID > {0}
)
