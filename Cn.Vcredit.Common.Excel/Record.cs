...
using ExcelExport = Cn.Vcredit.Common.Excel;
...
namespace Cn.Vcredit.Common.Excel
{
    internal class Record
    {
        ...
        public static Record Empty = new Record(Cn.Vcredit.Common.Excel.RID.Empty, new byte[0]);
        ...
        {
            _rid = Cn.Vcredit.Common.Excel.RID.ByteArray(rid);
            int offset = 0;
            ...
            while (bytesRemaining > 0)
            {
                ...
                if (continueIndex == -1)
                    ...
                    _continues.Add(new Record(Cn.Vcredit.Common.Excel.RID.CONTINUE, data.Get(offset, bytesToAppend)));
        ...
        internal static Bytes GetBytes(byte[] rid, Bytes data)
        {
            ...
            do
            {
                ...
                {
                    record.Append(Cn.Vcredit.Common.Excel.RID.CONTINUE);
                    record.Append(BitConverter.GetBytes(length));
                    record.Append(data.Get(offset, length));
                }
        ...
        public static List<Record> GetAll(Bytes stream)
        {
            ...
            {
                byte[] rid = Cn.Vcredit.Common.Excel.RID.ByteArray(stream.Get(i, 2).ByteArray);
