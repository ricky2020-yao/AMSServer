                ...
                foreach (XmlNode resultsNode in xmlNode.ChildNodes)
                {
                    ...
                    else if (resultsNode.Name == "Result")
                    {
                        ...
                        foreach (XmlNode resultNode in resultsNode.ChildNodes)
                        {
                            ...
                            if (proInfoT != null)
                            {
                                if(proInfoT.PropertyType == typeof(DateTime?)
                                || proInfoT.PropertyType == typeof(DateTime))
                                {
                                    DateTime dt;
                                    if (DateTime.TryParse(resultNode.InnerText, out dt))
                                    {
                                        proInfoT.SetValue(t, dt, null);
                                    }
                                }
                                else if (proInfoT.PropertyType.IsGenericType
                                    && proInfoT.PropertyType.GetGenericTypeDefinition().Equals(typeof(Nullable<>)))
                                {
                                    proInfoT.SetValue(t, string.IsNullOrEmpty(resultNode.InnerText)
                                        ? null : Convert.ChangeType(resultNode.InnerText, Nullable.GetUnderlyingType(proInfoT.PropertyType)), null);
                                }
                                else
                                {
                                    proInfoT.SetValue(t,
