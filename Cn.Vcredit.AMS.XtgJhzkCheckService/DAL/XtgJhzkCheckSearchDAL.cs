...
namespace Cn.Vcredit.AMS.XtgJhzkCheckService.DAL
{
    ...
    /// </summary>
    public class XtgJhzkCheckSearchDAL<T>
        : BaseSearchDAL<T> where T : class, new()
    {
        ...
        /// <returns></returns>
        public decimal GetSumAmount(BaseFilter baseFilter)
        {
            string sql = GetSumAmountSql(baseFilter);
            if (string.IsNullOrEmpty(sql))
                return 0;

            // 获取总金额
            return (decimal)QueryScalar(sql
        ...
        /// <param name="baseFilter"></param>
        /// <returns></returns>
        private string GetSumAmountSql(BaseFilter baseFilter)
        {
            ...
            sb.Append(" SELECT SUM(Amount) AS TOTALAMOUNT");
        ...
        protected override string GetSearchSql(BaseFilter baseFilter)
        {
            ...
            sb.Append(" SELECT * ");
            sb.Append("  FROM ( ");
            sb.Append(" SELECT ROW_NUMBER() OVER(ORDER BY Bid) AS RowId");
            sb.Append("        ,Bid ");
