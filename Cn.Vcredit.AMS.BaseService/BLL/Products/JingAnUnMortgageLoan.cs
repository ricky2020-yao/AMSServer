...
namespace Cn.Vcredit.AMS.BaseService.BLL.Products
{
    ...
    /// 静安小贷无抵押
    /// </summary>
    public class JingAnUnMortgageLoan: VcreditProduct
    {
        ...
        public List<List<EnumCostSubject>> Subjects
		{
			get
			{
				if (_Subjects == null)
				{
					...
					{ 
                        //手续费
                        new	List<EnumCostSubject>{EnumCostSubject.Procedures},
                        //本金、利息、管理费
        ...
        public JingAnUnMortgageLoan(Business business) : base(business) { }
        #endregion

        #region- 功能函数 -
		/// <summary>
		/// 创建基本科目（包括本金、利息、服务费、管理费）
		/// </summary>
		...
		public override List<BillItem> CreateBaseItems(bool isLast = false)
		{
            ...
            {
                decimal manageFee = SubjectFormula.CalculatInterest(Business.LoanCapital, Business.ManagementRate.Value);
                billItem.Add(CreateBillItem(EnumCostSubject.Manage, manageFee));
            }
		...
		public override List<BillItem> CreateBuckleFail(DateTime limitTime)
		{
			...
			// 判断当期帐单是否应生成本息扣失
			List<byte> subjects = new List<byte> { (byte)EnumCostSubject.Capital, 
		...
		public override BillItem CreatePunitiveInterest(DateTime limitTime, List<PenaltyInt> penaltyInts)
		{
            ...
            var subjects = new List<byte> { (byte)EnumCostSubject.Capital, (byte)EnumCostSubject.Interest, (byte)EnumCostSubject.Manage };
        ...
        public override Dictionary<EnumContractSide, string> GetProductCompanys()
        {
            ...
            dic.Add(EnumContractSide.Lending, Business.LendingSideKey);
        ...
        {
            return Singleton<SubjectDictionary>.Instance.JingAnUnMortgageLoanDic;
        }
        ...
        public override int GetAccountPayeeID(EnumCostSubject subject)
        {
            ...
            if (subject == EnumCostSubject.ServiceFee || subject == EnumCostSubject.ServiceBuckleFail)
                //服务费、服务费扣失扣到服务方
                return Business.ServiceSideID;
            else
                //其他扣到放贷方
        ...
        public override List<byte> ServiceBuckleFailSubjects
        {
            ...
            {
                // 判断当期帐单是否应生成本息扣失
                List<byte> subjects = new List<byte> { (byte)EnumCostSubject.ServiceFee };
