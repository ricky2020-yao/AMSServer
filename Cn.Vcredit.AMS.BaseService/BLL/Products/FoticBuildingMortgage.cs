...
{
    public class FoticBuildingMortgage:VcreditProduct
    {
        #region- 字段属性 -
        /// <summary>
        /// 科目优先级
        ...
        public FoticBuildingMortgage(Business business) : base(business) { }
        #endregion
        ...
        public override List<byte> PunitiveInterestSubjects
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(Business.GuaranteeSideKey))
                {
                    // 获取本金、利息或租金等参与科目，计算上期所欠本息或租金
                    subjects.AddRange(
                        new List<byte> { (byte)EnumCostSubject.Capital, 
                        (byte)EnumCostSubject.Interest,(byte)EnumCostSubject.ServiceFee });
                }
                else
                {
                    // 获取本金、利息或租金等参与科目，计算上期所欠本息或租金
